...
const MAIL_WORDS: &[&str] = &[
    "inbox", "mailbox", "my mail", "my email", "my e-mail", "my emails", "email", "emails",
    "e-mail", "junk", "spam",
];
...
fn mail_wanted(text: &str, forbids: &[Forbid]) -> Option<Mail> {
    ...
    let may_file = !forbids.contains(&Forbid::MovingMail)
        && FILING_WORDS.iter().any(|w| text.contains(w));
    Some(Mail { may_file })
}
...
const NEGATIONS: &[&str] = &[
    "never ", "do not ", "don't ", "dont ", "must not ", "no need to ", "without ", "not to ",
];
...
fn forbidden(text: &str) -> Vec<Forbid> {
    ...
    for neg in NEGATIONS {
        ...
        while let Some(found) = text[from..].find(neg) {
            ...
            if ["send", "reply", "write to", "text ", "tell anyone", "email anyone"]
                .iter()
                .any(|v| clause.contains(v))
                || ["message anyone", "message anybody", "message them", "message my"]
                    .iter()
                    .any(|v| clause.contains(v))
            {
                push(Forbid::Messaging);
            }
            if ["buy", "order", "pay", "purchase", "spend", "checkout", "check out"]
                .iter()
                .any(|v| clause.contains(v))
            {
...
mod tests {
    ...
    fn a_prohibition_beats_the_thing_it_forbids() {
        ...
        assert!(s.forbids.contains(&Forbid::OpeningBodies), "{:?}", s.forbids);
    }
    ...
    fn only_sites_that_are_actually_written_down_are_offered() {
        ...
        assert!(infer("Show me the website of CDV Software").domains.is_empty());
    }
